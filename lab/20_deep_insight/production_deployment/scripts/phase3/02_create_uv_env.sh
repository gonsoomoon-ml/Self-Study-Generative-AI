...
# Navigate to project root (3 levels up from phase3/)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
cd "$PROJECT_ROOT"

print_info "Project Root: $PROJECT_ROOT"
print_info "Phase3 Dir: production_deployment/scripts/phase3/"
...
# 심링크 생성 (상대 경로 사용)
ln -s production_deployment/scripts/phase3/pyproject.toml . || {
    print_error "pyproject.toml 심링크 생성 실패"
    exit 1
}

ln -s production_deployment/scripts/phase3/.venv . || {
    print_error ".venv 심링크 생성 실패"
    exit 1
}

if [ -f "production_deployment/scripts/phase3/uv.lock" ]; then
    ln -s production_deployment/scripts/phase3/uv.lock . || {
...
echo ""
echo "=== 파일 정보 ==="
echo "- pyproject.toml: 프로젝트 설정 및 의존성 (production_deployment/scripts/phase3/에서 관리, 루트에는 심링크)"
