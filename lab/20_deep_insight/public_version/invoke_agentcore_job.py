#!/usr/bin/env python3
"""
AgentCore Runtime Job Invoker - 독립 실행 가능한 스크립트

이 스크립트를 여러 번 실행하면 각각 독립적인 잡이 생성되어
여러 Fargate 컨테이너가 동시에 실행되는 것을 테스트할 수 있습니다.

사용법:
    python invoke_agentcore_job.py

테스트 목적:
    - 여러 잡이 각각 별도의 Fargate 컨테이너를 생성하는지 확인
    - 잡 완료 후 컨테이너가 자동으로 종료되는지 확인
"""
...
# 설정
AGENT_ARN = "arn:aws:bedrock-agentcore:us-east-1:057716757052:runtime/bedrock_manus_runtime-E8I6oFGlTA"
REGION = "us-east-1"
CLOUDWATCH_LOG_GROUP = "/aws/bedrock-agentcore/client-errors"  # 클라이언트 에러 로그 그룹
CLOUDWATCH_LOG_STREAM_PREFIX = "invoke-job-E8I6oFGlTA"  # 로그 스트림 접두사 (runtime ID 포함)

# 프롬프트 복잡도별 예상 시간
# PROMPT = "./data/Dat-fresh-food-claude.csv 파일의 총 매출액 계산해줘. PDF 보고서는 만들지 마."  # 2-5분 (가장 빠름)
