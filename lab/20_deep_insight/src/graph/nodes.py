...
async def coordinator_node(task=None, **kwargs):
    ...
    with tracer.start_as_current_span("coordinator") as span:
        ...
        if isinstance(task, dict):
            ...
            csv_file_path = task.get("csv_file_path")  # CSV 파일 경로 추출
        else:
        ...
        # CSV 파일 경로 추가 (있으면)
        if csv_file_path:
            shared_state['csv_file_path'] = csv_file_path
...
async def supervisor_node(task=None, **kwargs):
    ...
    with tracer.start_as_current_span("supervisor") as span:  
        ...
        agent = strands_utils.get_agent(
            ...
            tools=[coder_agent_fargate_tool, reporter_agent_fargate_tool, tracker_agent_tool, validator_agent_fargate_tool],  # Add coder, reporter, tracker and validator agents as tools
            streaming=True,
        )
