...
TOOL_SPEC = {
    "name": "reporter_agent_fargate_tool",
    "description": "Generate comprehensive reports based on analysis results using a specialized reporter agent running on AWS Fargate. This tool provides access to a reporter agent that can read analysis results from artifacts, create structured reports with visualizations, and generate output in multiple formats (HTML, PDF, Markdown) in isolated Fargate containers.",
    "inputSchema": {
        "json": {
            "type": "object",
            "properties": {
...
def handle_reporter_agent_fargate_tool(task: Annotated[str, "The reporting task or instruction for generating the report."]):
    """
    Generate comprehensive reports based on analysis results using a specialized reporter agent running on AWS Fargate.

    This tool provides access to a reporter agent that can:
    - Read analysis results from artifacts directory in Fargate containers
    ...
    with tracer.start_as_current_span("reporter_agent_fargate_tool") as span:
        print()  # Add newline before log
        logger.info(f"\n{Colors.GREEN}Reporter Agent Fargate Tool starting task{Colors.END}")
        logger.info(f"{Colors.BLUE}🚀 Using AWS Fargate for isolated report generation{Colors.END}")
        ...
        # Create reporter agent with Fargate-enabled tools using consistent pattern
        logger.info(f"{Colors.BLUE}📦 Creating reporter agent with Fargate tools{Colors.END}")
        reporter_agent = strands_utils.get_agent(
            ...
            system_prompts=apply_prompt_template(
                ...
                prompt_context={
                    ...
                    "FULL_PLAN": full_plan,
                    "EXECUTION_ENVIRONMENT": "AWS Fargate (isolated containers with automatic S3 upload for PDFs and charts)"
                }
            ),
            agent_type="claude-sonnet-3-7", # claude-sonnet-3-5-v-2, claude-sonnet-3-7
            enable_reasoning=False,
            prompt_cache_info=(True, None), # enable prompt caching for reporter agent
        ...
        async def process_reporter_stream():
            full_text = ""
            logger.info(f"{Colors.BLUE}🔄 Processing reporter agent with Fargate backend{Colors.END}")
            async for event in strands_utils.process_streaming_response_yield(
                reporter_agent, message, agent_name="reporter", source="reporter_fargate_tool"
            ):
        ...
        logger.info(f"\n{Colors.GREEN}Reporter Agent Fargate Tool completed successfully{Colors.END}")
        logger.info(f"{Colors.BLUE}✅ Reports generated in isolated Fargate environment{Colors.END}")
...
def reporter_agent_fargate_tool(tool: ToolUse, **_kwargs: Any) -> ToolResult:
    tool_use_id = tool["toolUseId"]
    task = tool["input"]["task"]

    # Use the existing handle_reporter_agent_fargate_tool function
    result = handle_reporter_agent_fargate_tool(task)
...
handle_reporter_agent_tool_fargate = handle_reporter_agent_fargate_tool