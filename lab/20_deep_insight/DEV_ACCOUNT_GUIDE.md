...
```bash
1️⃣ python3 01_create_agentcore_runtime.py  # Runtime 생성
   ↓
   (02_agentcore_runtime.py는 Container에서 자동 실행됨 - 직접 실행 안 함!)
   ↓
2️⃣ python3 03_invoke_agentcore_job_vpc.py  # Runtime 테스트
```
...
### 2.1 01_create_agentcore_runtime.py 실행
...
**간단한 테스트** (2-5분, 권장):
```python
# 03_invoke_agentcore_job_vpc.py 편집
...
### 3.2 03_invoke_agentcore_job_vpc.py 실행
...
- [ ] 01_create_agentcore_runtime.py 실행 성공
- [ ] Runtime 상태: READY
- [ ] `.env`에 RUNTIME_ARN 추가됨
- [ ] 03_invoke_agentcore_job_vpc.py 실행 성공
- [ ] 스트리밍 응답 수신 완료
- [ ] 총 매출액 계산 결과 출력
...
### 01_create_agentcore_runtime.py 실행 후
...
1. **Git에 커밋** (설정 파일 제외)
   ...
   git add 01_create_agentcore_runtime.py
   git add 02_agentcore_runtime.py
   git add 03_invoke_agentcore_job_vpc.py
...
| 파일 | 역할 | 실행 여부 |
|------|------|-----------|
| `01_create_agentcore_runtime.py` | Runtime 생성 | ✅ **직접 실행** |
| `02_agentcore_runtime.py` | Runtime 진입점 | ❌ Container에서 자동 실행 |
| `03_invoke_agentcore_job_vpc.py` | Runtime 테스트 | ✅ **직접 실행** |
