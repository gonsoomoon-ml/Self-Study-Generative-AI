...
def research_node(state: State) -> Command[Literal["supervisor"]]:
    ...
    logger.info(f"{Colors.GREEN}🔍 ===== Research completed task ====={Colors.END}")
...
def code_node(state: State) -> Command[Literal["supervisor"]]:
    ...
    logger.info(f"{Colors.GREEN}💻 ===== Code completed task ====={Colors.END}")
    return Command(
        update={
...
def browser_node(state: State) -> Command[Literal["supervisor"]]:
    ...
    logger.info(f"{Colors.GREEN}🌐 ===== Browser agent starting task ====={Colors.END}")
    ...
    logger.info(f"{Colors.GREEN}🌐 ===== Browser completed task ====={Colors.END}")
...
def planner_node(state: State) -> Command[Literal["supervisor", "__end__"]]:
    ...
    if state.get("deep_thinking_mode"): llm = get_llm_by_type(AGENT_LLM_MAP["planner"])
...
def coordinator_node(state: State) -> Command[Literal["planner", "__end__"]]:
    ...
    logger.info(f"{Colors.GREEN}🗣️ ===== Coordinator talking...... ====={Colors.END}")
    ...
    logger.info(f"{Colors.GREEN}🗣️ ===== Coordinator completed task ====={Colors.END}")
    return Command(
...
def validator_node(state: State) -> Command[Literal["supervisor"]]:
    """Validator node that validates calculations and generates citations."""
    logger.info(f"{Colors.GREEN}✅ ===== Validator validating calculations ====={Colors.END}")
    # logger.debug(f"\n{Colors.RED}Validator - current state messages:\n{pprint.pformat(state['messages'], indent=2, width=100)}{Colors.END}")

    # Create validator agent
    validator_agent = create_react_agent_langfuse(agent_name="validator")
    
    # Call validator agent with state (same pattern as other agents)
    result = validator_agent.invoke(state=state)
    
    # Extract response text from result
    response_text = result["content"][-1]["text"]
    # logger.debug(f"\n{Colors.RED}Validator response:\n{response_text}{Colors.END}")

    # Update clues with validator output
    clues = state.get("clues", "")
    clues = '\n\n'.join([clues, CLUES_FORMAT.format("validator", response_text)])

    # Update state with validator response  
    history = state.get("history", [])
    history.append({"agent": "validator", "message": response_text})

    logger.info(f"{Colors.GREEN}✅ ===== Validator completed task ====={Colors.END}")
    
    return Command(
        update={
            "messages": [get_message_from_string(role="user", string=RESPONSE_FORMAT.format("validator", response_text), imgs=[])],
            "messages_name": "validator", 
            "history": history,
            "clues": clues
        },
        goto="supervisor",
