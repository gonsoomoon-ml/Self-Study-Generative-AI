    ...
    def wrapped_bedrock_converse(self, **kwargs):
        ...
        try:
            if self.llm.stream == True:
                ...
                print("### ai_message: \n", json.dumps(ai_message, indent=2, ensure_ascii=False))

                # response = self.chain_no_stream( ## pipeline의 제일 처음 func의 argument를 입력으로 한다. 여기서는 converse_api의 arg를 쓴다.
                #     llm=self.llm,
                #     system_prompts=system_prompts,
                #     messages=messages,
                #     tool_config=tool_config,
                #     verbose=self.verbose
                # )
                # ai_message = {"ai_message": "no stream"}
            ...
            print("Error in exception: ", error_message)
        ...
        try:
            if self.llm.stream == True:
                ...
                # else:
                #     response_text = str(content_list[0]) if content_list else ""
                #     # 필요하다면, 여기서 logger.warning("text 키가 없습니다...") 등으로 로깅
