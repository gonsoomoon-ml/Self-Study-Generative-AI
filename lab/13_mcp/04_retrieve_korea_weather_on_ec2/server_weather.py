...
# EC2 Test
########################################################
# mcp = FastMCP("kr-weather", host="0.0.0.0", port=8000)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        ...
        # run_tests()
        print("[DEBUG] Starting MCP server on http://localhost:8000/mcp",file=sys.stderr)
        mcp.run(transport="streamable-http")
    else:
        # MCP 서버 모드: stdout 완전 차단 (디버깅 시 주석처리)
        # sys.stdout = open(os.devnull, "w")
        print("[DEBUG] Starting MCP server on http://0.0.0.0:8000/mcp",file=sys.stderr)
        mcp.run(transport="streamable-http")
